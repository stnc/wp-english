...
$StncBabOneEng_meta_key_video = 'stnc-babOne-EngMeta';
$StncBabOneEng_postID = isset($_GET['post']) ? $_GET['post'] : null;//post  id  for edit
$StncBabOneEng_get_post_type_ = (get_post_type($StncBabOneEng_postID));//get type
$StncBabOneEng_post_type_new = isset($_REQUEST['post_type']) ? $_REQUEST['post_type'] : 'post';//for new
 ...
 */
class StncBabOneEng_metabox_engine
{
	public $nonce = 'st_studio1';
	public $meta_key;
	private $fields;
	private $current_id;
	private $SaveMethodSingle;

	public function __construct($fields, $StncBabOneEng_meta_key_video, $SaveMethodSingle = false)
	{
// print_r($fields);
//

		$this->meta_key = $StncBabOneEng_meta_key_video;
...
print_r($this->fields);
	...
	public function add_custom_meta_box()
	{
		foreach ($this->fields as $key => $field) {
			add_meta_box(
				...
				$field['page'],// Admin page (or post type)
				$field['context'],
				$field['priority'],
				$field
	...
	public function meta_box_output($post, $field_arg)
	{
		...
		if ($fields['title_h2']) {
			echo '<li><h2  data-required="StncBabOneEng_pageSetting_background_repeat"><strong>' . $fields['title'] . '</strong></h2> </li>';
		}
        ...
        <div class="background_attachment_metabox_container">';
				...
				case 'media-gallery':
					...
					echo '<li class="' . $values['class_li'] . '" id="' . $values['name'] . '_li">
	                            ...
	                            <input type="button" name="uploader" class="STNCupload_button button button-primary" value="' . __('Add Images', 'StncBabOneEng') . '">
	                            </div>
	                            </div>
	                            </div>
	                      </li>';
...
function StncBabOneEng_video_options_()
{
	include('metabox_options.php');
	$metaBoxOption['0'] = $StncBabOneEng_OptionsPageSettingvideo;
	$engine_page_video = new StncBabOneEng_metabox_engine($metaBoxOption, 'stnc-babOne-EngMeta', true);
// $engine_page_video = new StncBabOneEng_metabox_engine($ch_post_options_video, 'stnc-babOne-EngLocation-Setting', true);

}


if ($StncBabOneEng_get_post_type_ == 'stnc-babone-eng') {
	StncBabOneEng_video_options_();
}


if ($StncBabOneEng_post_type_new == 'stnc-babone-eng') {
	StncBabOneEng_video_options_();
}
