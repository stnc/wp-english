...
/***SIDEBAR Doctor select METABOX (ONLY video )****/
function CHfw_doctor_selected_get_metaOLD($value)
{
	global $post;

	$field = get_post_meta($post->ID, 'CHfw_DoctorAndDepartmant_ForSinglevideoPage', true);
	if (!empty($field)) {
...
{
	add_meta_box(
		'CHfw_DoctorAndDepartmant_ForSinglevideoPage_metabox',
...
function CHfw_doctor_selected_html($post)
{
    ...
    <label for="CHfw_DrAndDep_program_and_services">
		<?php _e('Programs and Services:', 'CHfw-video'); ?>
    </label>
    <br>
	<?php
    ...
    <label for="CHfw_DrAndDep_display_doctor_calendar">
		<?php
		_e('Calendar to Display Doctor:', 'CHfw-video');
...
/*register metabox */
function CHfw_video_init_metabox()
{
	// add meta box
