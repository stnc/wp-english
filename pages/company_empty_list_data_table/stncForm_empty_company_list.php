 ...
 */
class Stnc_wp_floor_Empty_List_Table extends WP_List_Table
{
	...
	public function get_columns()
	{
		return array(
			...
			'door_number'   => __('Door number', 'admin-table-tut'),
	 ...
	 **************************************************************************/
	function column_bina($item)
	{
		$delete_nonce = wp_create_nonce('sp_delete_stncMapFloors');

		$title = '<strong>' . $item['bina'] . '</strong>';
	...
	public static function get_stncMapFloorss($per_page = 5, $page_number = 1)
	{
		...
		if (isset($_POST['s'])) {
		    ...
		    $sql = "SELECT loc.*,build.name AS build_name,floors.name AS floors_name,CONCAT(build.name , ' - ',  floors.name) as bina ,loc.door_number
			...
			INNER JOIN  {$wpdb->prefix}stnc_map_floors AS floors  ON  loc.floor_id=floors.id  and loc.is_empty=1
	...
	public static function record_count()
	{
		...
		$sql = "SELECT COUNT(*) FROM {$wpdb->prefix}stnc_map_floors_locations as loc where loc.is_empty=1";
...
<?php
	...
	public function get_sortable_columns()
	{
		...
		return array(
			...
			'door_number' => array('door_number', false),
 ...
 */
function stnc_wp_floor_render_empty_list_page()
{

	//Create an instance of our package class...
	$testListTable = new Stnc_wp_floor_Empty_List_Table();
