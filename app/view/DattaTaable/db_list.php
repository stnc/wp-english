 ...
 */
class Helix_wp_word_list_table extends WP_List_Table
{
	...
	function column_main_language($item)
	{
		...
		$actions = [
			...
			'edit' => sprintf('<a href="?page=helix_language_editor&trigger=edit&id=%s&_wpnonce=%s">%s</a>',  absint($item['id']), $delete_nonce, $editLangs)
		];

		return $title . $this->row_actions($actions);
	}

	//http://summit.test/wp-admin/admin.php?page=wp_list_table_class&action=delete&helixMaps=16&_wpnonce=243e56ab02
	...
	public function prepare_items()
	{
		...
		$this->items = self::get_helixMapss($per_page, $current_page);
	}

	/**
	 * Retrieve helixMapss data from the database
	 *
	 * @param int $per_page
	 ...
	 * @return mixed
	 */
	public static function get_helixMapss($per_page = 5, $page_number = 1)
	{
	...
	public static function record_count()
	{
		...
		$sql = "SELECT COUNT(*) FROM {$wpdb->prefix}helix_words";
	...
	public function process_bulk_action()
	{
		...
		if ('delete' === $this->current_action()) {
			...
			if (!wp_verify_nonce($nonce, 'sp_delete_helixMaps')) {
				die('Go get a life script kiddies');
			} else {
				self::delete_helixMaps(absint($_GET['helixMaps']));
		...
		if ('view' === $this->current_action()) {
			...
			// In our file that handles the request, verify the nonce.
			$nonce = esc_attr($_REQUEST['_wpnonce']);
			$id = filter_input(INPUT_GET, 'helixMaps', FILTER_DEFAULT);
			$data = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}helix_map_s_locations WHERE id = $id");
			// print_r(	$data );
			...
			<div id="advanced" class="postbox ">
				<div class="inside">
					<div class="card shadow1" style="max-width:100%!important">
						...
						<div><mark class="dont"></mark> <a href="/wp-admin/admin.php?page=helix_language_editor_explode&trigger=show&building_id=<?php echo $data->building_id; ?>&_id=<?php echo $data->_id; ?>&id=<?php echo $data->id; ?>"><?php esc_html_e('Edit', 'helix-lng') ?></a> </div>
						<hr>
...
function helix_DataTABLE_page()
{
	...
	$testListTable = new Helix_wp_word_list_table();
